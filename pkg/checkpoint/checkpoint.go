...
// CheckpointPodDevicesEntry is representing Pod specific deviceID allocations from kubelet checkpoint file structure - valid until K8s 1.20
...
// NewCeckpointPodDevicesEntry is representing Pod specific deviceID allocations from kubelet checkpoint file structure - valid from K8s 1.21 onward
// Reference: https://github.com/kubernetes/kubernetes/commit/a8b8995ef241e93e9486d475126450f33f24ef4e
type NewCeckpointPodDevicesEntry struct {
	PodUID        string
	ContainerName string
	ResourceName  string
...
// CheckpointFile is representing the kubelet checkpoint file structure with only relevant fields - valid until K8s 1.20
// TODO: REMOVE THIS TPYE AFTER 1.20 SUPPORT IS DROPPED
...
// NewCheckpointFile is representing the kubelet checkpoint file structure with only relevant fields - valid from K8s 1.21 onward
// Reference: https://github.com/kubernetes/kubernetes/commit/a8b8995ef241e93e9486d475126450f33f24ef4e
type NewCheckpointFile struct {
	Data struct {
		PodDeviceEntries []NewCeckpointPodDevicesEntry
	}
}
...
func TranslateNewCheckpointToOld(newFile NewCheckpointFile) CheckpointFile {
	var oldFile CheckpointFile
	oldFile.Data.PodDeviceEntries = make([]CheckpointPodDevicesEntry, len(newFile.Data.PodDeviceEntries))
